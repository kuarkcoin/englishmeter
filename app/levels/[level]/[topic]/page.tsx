...
export default function LevelTopicPage() {
  ...
  const level = (Array.isArray(params.level) ? params.level[0] : params.level || '').toLowerCase();
  const topicParam = (Array.isArray(params.topic) ? params.topic[0] : params.topic || '') as string;

  // Şimdilik sadece A1 için topic quiz çalışıyor
  if (level !== 'a1') {
    return (
