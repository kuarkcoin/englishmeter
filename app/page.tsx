...
function startTest(testSlug: string) {
  // 1) GRAMMAR FOCUS → JSON’dan direkt /quiz’e gitsin
  if (slugToTag[testSlug]) {
    const attemptId = `session-${Date.now()}-${Math.random()
      .toString(36)
      .substr(2, 9)}`;
    ...
    // 🔹 BURADA ARTIK isCorrect İLE GELİYOR → skor ekranı düzgün çalışır
  ...
  // 2) DİĞER TESTLER (Quick, Mega, Vocab, IELTS) → /start ROUTE'U KULLANSIN
            ...
            <div className="relative mx-auto max-w-sm">
              <div className="rounded-3xl bg-gradient-to-br from-blue-600 to-indigo-500 p-6 shadow-2xl text-white">
                ...
                <div className="text-sm opacity-90 mb-4">
                  Upper-Intermediate · 78% accuracy
                </div>
      ...
      <div className="flex flex-col items-center justify-center px-4 pb-24 pt-4">
        <div id="all-tests" className="w-full max-w-6xl mx-auto text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 mb-16">
            ...
            {/* IELTS: Global Race’in altında, açık mavi, aynı ebat */}
            <button
              onClick={() => startTest(ieltsTest.slug)}
              className="flex items-center justify-center px-6 py-8 rounded-2xl bg-sky-400 text-white text-xl font-bold shadow-xl hover:bg-sky-500 transition-all lg:col-start-4 lg:row-start-2"
            >
              {ieltsTest.title}
            </button>
          </div>
