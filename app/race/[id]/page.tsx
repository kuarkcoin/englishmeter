...
export default function RaceExamPage() {
  ...
  // --- 1. START EXAM ---
  const startExam = () => {
    // Format questions with Answer Normalization
    const formattedQuestions: Question[] = (questionsData as any[]).map((q) => ({
      id: q.id,
        ...
        <div className="bg-white p-10 rounded-2xl shadow-xl max-w-md w-full text-center border border-gray-100">
          ...
          <p className="text-gray-600 mb-8 text-lg">
            ...
            <span className="text-sm font-bold text-green-600">
              Currently {totalParticipants} people racing...
            </span>
          </p>
      ...
      <main className="min-h-screen flex items-center justify-center bg-blue-50 p-4">
        <div className="bg-white p-8 rounded-2xl shadow-2xl max-w-md w-full text-center animate-fade-in">
          ...
          <div className="bg-blue-100 p-4 rounded-xl mb-6 mt-4">
            <span className="block text-sm text-blue-600 font-bold uppercase">
              Your Score
            </span>
            <span className="text-5xl font-black text-blue-700">{score} / 50</span>
          </div>
          <p className="text-left text-sm font-bold text-gray-700 mb-2 ml-1">
            Enter name to see global ranking:
          </p>
          <input
            type="text"
            autoFocus
      ...
      <main className="min-h-screen bg-gray-50 p-4 md:p-8">
        <div className="max-w-2xl mx-auto bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-200">
          <div className="bg-blue-600 p-8 text-white text-center relative">
            <span className="absolute top-4 left-4 bg-blue-800 text-xs px-2 py-1 rounded">
              RACE #{raceId}
            </span>
              ...
              {topLeaderboard.map((user, i) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <span
                      className={`font-bold ${
                        user.isRealUser ? "text-black" : "text-gray-600"
                      }`}
                    >
            ...
            {userResult && (
              <div className="mb-8">
                <div className="py-2 text-center text-xs text-gray-400">
                  ... {userResult.rank - 21} racers hidden ...
                </div>
      ...
      <div className="w-full max-w-3xl flex justify-between items-center mb-6 bg-white p-4 rounded-xl shadow-sm border border-gray-200">
        ...
        <div className="text-right">
          ...
          <div className="text-xl font-bold text-blue-600">
            {currentQIndex + 1}{" "}
            <span className="text-gray-300 text-base">/ 50</span>
          </div>
        </div>
      </div>
          ...
          {currentQ.options.map((opt, i) => {
            ...
            return (
              <button
                ...
                className="group relative w-full text-left p-5 border-2 border-gray-100 rounded-xl
                           hover:border-blue-500 hover:bg-blue-50 transition-all duration-200 active:scale-[0.99]
